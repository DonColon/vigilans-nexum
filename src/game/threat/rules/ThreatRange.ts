...
import { tileKey, blockedTiles, occupiedTiles, reachableTiles, attackableTiles } from "@/game/movement/rules/Pathfinding";
import { InventoryKind, UnitData } from "@/game/units/components/UnitComponent";
...
/*
 * What the other side covers, in the same pure form the pathfinding is kept
 ...
 */
export function weaponReach(unit: UnitData): WeaponReach | null {
 ...
 */
export function threatRangeOf(grid: GridData, unit: ThreatUnit, others: readonly UnitLocation[]): ThreatRange {
	const blocked = blockedTiles(others, unit.data);
	const occupied = occupiedTiles(others, unit.data.id);
	const reachable = reachableTiles(grid, unit.tile, unit.data.stats.movement, blocked, occupied);
	const reach = weaponReach(unit.data);

	return {
		movement: reachable.map((tile) => ({ column: tile.column, row: tile.row })),
		attack: reach === null ? [] : attackableTiles(grid, reachable, reach.minRange, reach.maxRange)
	};
}
 ...
 */
export function threatRangeOfAll(grid: GridData, units: readonly ThreatUnit[], others: readonly UnitLocation[]): ThreatRange {
	const movement = new Map<string, GridPositionData>();
	const attack = new Map<string, GridPositionData>();

	for (const unit of units) {
		const range = threatRangeOf(grid, unit, others);

		for (const tile of range.movement) {
			movement.set(tileKey(tile.column, tile.row), tile);
		}

		for (const tile of range.attack) {
			attack.set(tileKey(tile.column, tile.row), tile);
		}
	}
