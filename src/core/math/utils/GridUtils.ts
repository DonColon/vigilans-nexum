...
export class GridUtils {
	...
	public static worldToGrid(worldPos: Vector2D, cellSize: number): Vector2D {
		return new Vector2D(Math.floor(worldPos.x / cellSize), Math.floor(worldPos.y / cellSize));
	}
	...
	public static gridToWorld(gridPos: Vector2D, cellSize: number): Vector2D {
		return new Vector2D(gridPos.x * cellSize + cellSize / 2, gridPos.y * cellSize + cellSize / 2);
	}
	...
	public static gridToWorldTopLeft(gridPos: Vector2D, cellSize: number): Vector2D {
		return new Vector2D(gridPos.x * cellSize, gridPos.y * cellSize);
	}
	...
	public static worldToIsometric(worldPos: Vector2D): Vector2D {
		return new Vector2D(worldPos.x - worldPos.y, (worldPos.x + worldPos.y) / 2);
	}
	...
	public static isometricToWorld(isoPos: Vector2D): Vector2D {
		return new Vector2D((isoPos.x + isoPos.y * 2) / 2, (isoPos.y * 2 - isoPos.x) / 2);
	}
