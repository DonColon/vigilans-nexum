...
export class Ellipse implements Shape {
	...
	public contains(point: Vector2D): boolean {
		...
		const normalized = (localPoint.x * localPoint.x) / (this.radiusX * this.radiusX) + (localPoint.y * localPoint.y) / (this.radiusY * this.radiusY);
	...
	private intersectsWithLine(line: Line): boolean {
		...
		const a = (dx * dx) / (this.radiusX * this.radiusX) + (dy * dy) / (this.radiusY * this.radiusY);

		const b = 2 * ((localStart.x * dx) / (this.radiusX * this.radiusX) + (localStart.y * dy) / (this.radiusY * this.radiusY));

		const c = (localStart.x * localStart.x) / (this.radiusX * this.radiusX) + (localStart.y * localStart.y) / (this.radiusY * this.radiusY) - 1;
	...
	private intersectsWithCircle(circle: Circle): boolean {
		...
		const expandedEllipse = new Ellipse(this.center.x, this.center.y, this.radiusX + circleRadius, this.radiusY + circleRadius, this.rotation);
	...
	private intersectsWithRectangle(rectangle: Rectangle): boolean {
		...
		if (this.contains(corners.topLeft) || this.contains(corners.topRight) || this.contains(corners.bottomLeft) || this.contains(corners.bottomRight)) {
			return true;
		}

		// Check if any side intersects the ellipse
		const sides = rectangle.getSides();
		if (this.intersects(sides.top) || this.intersects(sides.right) || this.intersects(sides.bottom) || this.intersects(sides.left)) {
			return true;
		}
	...
	private intersectsWithEllipse(other: Ellipse): boolean {
		...
		const maxDistance = Math.max(this.radiusX, this.radiusY) + Math.max(other.radiusX, other.radiusY);
	...
	public getBounds(): Rectangle {
		if (this.rotation === 0) {
			...
			return new Rectangle(this.center.x - this.radiusX, this.center.y - this.radiusY, this.radiusX * 2, this.radiusY * 2);
		}
	...
	private transformToLocal(point: Vector2D): Vector2D {
		// Translate to origin
		const translated = new Vector2D(point.x - this.center.x, point.y - this.center.y);
		...
		return new Vector2D(translated.x * cos - translated.y * sin, translated.x * sin + translated.y * cos);
	}

	private transformToWorld(localPoint: Vector2D): Vector2D {
		if (this.rotation === 0) {
			return new Vector2D(localPoint.x + this.center.x, localPoint.y + this.center.y);
		}
		...
		const rotated = new Vector2D(localPoint.x * cos - localPoint.y * sin, localPoint.x * sin + localPoint.y * cos);

		// Translate to center
		return new Vector2D(rotated.x + this.center.x, rotated.y + this.center.y);
	}
}
