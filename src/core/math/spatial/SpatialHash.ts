...
export class SpatialHash {
	...
	public update(entity: ShapeEntity, oldBounds: Rectangle): void {
		...
		for (const oldCell of oldCells) {
			const isStillInCell = newCells.some((newCell) => newCell.x === oldCell.x && newCell.y === oldCell.y);
		...
		for (const newCell of newCells) {
			const isNewCell = !oldCells.some((oldCell) => oldCell.x === newCell.x && oldCell.y === newCell.y);
