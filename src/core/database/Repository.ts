...
export class Repository<Name extends StoreNames> {
	...
	public async create(data: StoreType<Name>): Promise<IDBValidKey> {
		return this.executeStoreOperation("readwrite", (store) => store.add(data));
	}

	public async read(key: IDBValidKey): Promise<StoreType<Name>> {
		return this.executeStoreOperation("readonly", (store) => store.get(key));
	}

	public async update(data: StoreType<Name>): Promise<IDBValidKey> {
		return this.executeStoreOperation("readwrite", (store) => store.put(data));
	}

	public async delete(key: IDBValidKey): Promise<boolean> {
		const result = await this.executeStoreOperation("readwrite", (store) => store.delete(key));
		return result === undefined;
	}

	public async list(): Promise<StoreType<Name>[]> {
		return this.executeStoreOperation("readonly", (store) => store.getAll());
	}
	...
	public async exists(data: StoreType<Name>): Promise<boolean> {
		...
		const result = await this.executeStoreOperation("readonly", (store) => store.getKey(key));
		return result !== undefined;
	}
	...
	private async executeStoreOperation<T>(mode: IDBTransactionMode, operation: (store: IDBObjectStore) => IDBRequest<T>): Promise<T> {
