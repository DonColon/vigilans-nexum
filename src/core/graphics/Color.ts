...
export class Color {
	private constructor(private readonly settings: ColorSettings) {}
	...
	public static css(property: string): Color {
		if (property.startsWith("#")) {
			...
			if (args !== null) {
				...
				return Color.rgb(red, green, blue, alpha * 100);
			}
			...
			if (args !== null) {
				...
				return Color.hwb(hue, whiteness, blackness, alpha * 100);
			}
		}

		throw new SyntaxError("Invalid color property");
	}

	public mix(other: Color, percentage: number = 50): Color {
		if (percentage < 0 || percentage > 100) {
			throw new RangeError("value must be percentage");
		}

		const p = percentage / 100;
		const r = Math.floor((1 - p) * this.settings.red + p * other.settings.red);
		const g = Math.floor((1 - p) * this.settings.green + p * other.settings.green);
		const b = Math.floor((1 - p) * this.settings.blue + p * other.settings.blue);

		if (this.settings.alpha !== undefined && other.settings.alpha !== undefined) {
			const a = (1 - p) * (this.settings.alpha / 100) + p * (other.settings.alpha / 100);
			return Color.rgb(r, g, b, a * 100);
		}
	...
	public asCss(property: "hex" | "rgb" | "hsl" | "hwb"): string {
		if (property === "hex") {
			...
			if (alpha) {
				return `rgb(${red} ${green} ${blue} / ${(alpha / 100).toFixed(1)})`;
			} else {
			...
			if (alpha) {
				return `hsl(${hue} ${saturation} ${lightness} / ${(alpha / 100).toFixed(1)})`;
			} else {
			...
			if (alpha) {
				return `hwb(${hue} ${whiteness} ${blackness} / ${(alpha / 100).toFixed(1)})`;
			} else {
