...
describe("GridUtils Test Suite", () => {
	...
	test("Gets 4-directional neighbors", () => {
		...
		expect(neighbors).toContainEqual(new Vector(5, 4)); // Up
		expect(neighbors).toContainEqual(new Vector(6, 5)); // Right
		expect(neighbors).toContainEqual(new Vector(5, 6)); // Down
		expect(neighbors).toContainEqual(new Vector(4, 5)); // Left
	});
	...
	test("Gets 8-directional neighbors including diagonals", () => {
		...
		expect(neighbors).toContainEqual(new Vector(4, 4)); // Diagonal
		expect(neighbors).toContainEqual(new Vector(6, 6)); // Diagonal
	});
	...
	test("Generates Bresenham line cells", () => {
		...
		const end = new Vector(4, 2);
		
		const cells = GridUtils.bresenhamLine(start, end);
		expect(cells.length).toBeGreaterThan(2);
		expect(cells[0]).toEqual(new Vector(0, 0));
		expect(cells[cells.length - 1]).toEqual(new Vector(4, 2));
	});
	...
	test("Gets cells in circle", () => {
		...
		expect(cells).toContainEqual(new Vector(0, 0)); // Center
		expect(cells).toContainEqual(new Vector(1, 0));
		expect(cells).toContainEqual(new Vector(0, 1));
	});
	...
	test("Gets cells in rectangle", () => {
		...
		expect(cells).toContainEqual(new Vector(0, 0));
		expect(cells).toContainEqual(new Vector(2, 1));
	});
});
